...
### 目的<br>
  ...
  現時点では、投資対象の株式銘柄は①東証プライム銘柄②東証グロース銘柄③東証スタンダード銘柄
  に限定する。
### ユースケース<br>
...
トリガー：株式売買
前提条件: 認可された管理者権限<br>
主要フロー: 株式売買情報をSrc/Report配下にhtml形式で出力する。レポートファイル形式はyyyymmdd_[発注ID]_[銘柄コード]_[金額].htmlとし、同階層フォルダのsummary.htmlにこれまでの全取引への参照を置く。<br>
事後条件: なし。
...
```
## 6. API / モジュール設計（Interface Design）
- API 仕様（REST/GraphQL）（リクエスト/レスポンス・エラーコード）<br>
